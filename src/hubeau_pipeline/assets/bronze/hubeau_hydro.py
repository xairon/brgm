"""
Asset Bronze - Hydrométrie Hub'Eau
Petite brique spécialisée pour l'API hydrométrie
"""
...
@asset(
    ...
    description="Récupération des données hydrométriques depuis l'API Hub'Eau"
)
def hydro_raw(context: AssetExecutionContext):
    """
    Petite brique : Récupère les données hydrométriques brutes
    
    Cette brique est responsable uniquement de :
    - Appeler l'API Hub'Eau hydrométrie
    ...
    # Configuration des endpoints Hub'Eau hydrométrie
    endpoints = [
        {
            "name": "stations",
            "url": "https://hubeau.eaufrance.fr/api/v1/hydrometrie/stations",
            "params": {"size": 1000}
        },
        {
            "name": "observations", 
        ...
        {
            "name": "chroniques",
            "url": "https://hubeau.eaufrance.fr/api/v1/hydrometrie/chroniques", 
    ...
    logger.info(f"✓ Hydrométrie {day}: {total_records} enregistrements au total")
    return result
